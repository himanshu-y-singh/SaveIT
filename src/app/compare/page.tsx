...
export default function DashboardPage() {
  ...
  const fetchProducts = useCallback(async (query: string) => {
    ...
    setProducts([]); 
    function mutateBase64(base: string) {
      const chars = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/';
      const randomChar = chars[Math.floor(Math.random() * chars.length)];
      return base.slice(0, -1) + randomChar;
    }
    try {
      const response = await fetch(`${API_BASE_URL}?lat=${LAT}&lon=${LON}&type=groupsearch&query=${encodeURIComponent(query)}`, {
        method: 'POST', // Use POST as per the working curl request
        headers: {
          'requies': '', // Add the 'requies' header as required
          'x-request-id': mutateBase64(
            'BQQGBgRRUwVLDFpRBkxVBVwATwAFWgAUWwUJDF1dXFcAUVdQ'
          ), // Replace with a dynamically generated or valid request ID
        },
        body: '', // Include an empty body as per the curl request
      });
      if (!response.ok) {
